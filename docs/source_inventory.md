...
mechanism; `nflreadpy`/`nflreadr` are thin wrappers around the same URLs).
...
| Depth charts (official, mirrored by nflverse) | Starter/backup ordering by position, per week | 2001–present | Weekly | `.../depth_charts/depth_charts_{year}.parquet` | Free | Medium — depth chart entries are self-reported by clubs and sometimes stale between real changes. **2021–2024 confirmed in the historical schema used by this pipeline (`season`/`week`/`depth_position`/`formation` columns). The 2026 file was pulled in a completely different schema** (`dt`/`pos_grp`/`pos_slot`/`pos_rank`, no season/week columns) — nflverse appears to have switched depth-chart sources for the current season. `src/build_player_week.py`'s `merge_depth_charts()` detects this and skips the file with a warning rather than crashing; depth-chart context is currently absent from live 2026 predictions as a result. Mapping the new schema in is a small, contained follow-up once it stabilizes. |
...
| Schedules (`schedules`) | Game metadata: kickoff time, network, roof, surface, divisional game flag, `gameday` — used to derive rest days and travel | 1999–present | Weekly | `.../nflverse-data/releases/download/schedules/games.csv` (via `nflreadpy.load_schedules`) | Free | High. Static per-game facts; roof/surface known well before kickoff so it's leakage-safe. |
